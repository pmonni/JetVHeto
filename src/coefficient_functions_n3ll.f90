module coefficient_functions_n3ll
...
contains
  ...
  ! Re-derived by LR with full nf dependence, now removing the fictious Lt dependence
  !----------------------------------------------------------------------
  function C2gg(y) result(res)
    ...
    case(cc_DELTA)
       res =  4._dp*((607._dp*CA**2)/648._dp - (41._dp*CA*nF)/324._dp -  (67._dp*CA**2*Pi**2)/576._dp + &
       &  (5._dp*CA*nF*Pi**2)/288._dp + (5._dp*CA**2*Pi**4)/1152._dp - (77._dp*CA**2*Zeta3)/144._dp + &
       &  (7._dp*CA*nF*Zeta3)/72._dp)
    end select
  ...
  subroutine InitCoeffMatrix(grid, C1, C2, G1)
    ...
    ! All matrices are required: the initializer accesses each unconditionally.
    type(split_mat), intent(inout) :: C2,G1
...
end module coefficient_functions_n3ll
